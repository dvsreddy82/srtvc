...
export class MobileLocalStorageService {
  private dbName: string = 'pet-management.db';
  private db: any = null;
  ...
  private async getAllItems(table: string): Promise<any[]> {
    const [results] = await this.db.executeSql(`SELECT data FROM ${table}`);
...
export const localStorageService = new MobileLocalStorageService();
